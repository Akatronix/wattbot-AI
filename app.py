...
import random
import json
import time  # <-- ADDED IMPORT FOR SELF-HEALING
from flask import Flask, request, jsonify
from google import genai
from google.genai import types
from flask_cors import CORS
...
# ==============================================================================
# --- KEY ROTATION LOGIC (IMPROVED WITH SELF-HEALING) ---
# ==============================================================================

# Store the timestamp when a key was rate-limited
rate_limited_keys = {} # e.g., {"key1": 1678886400.123}

# Define the time limit for a per-minute rate limit (65 seconds to be safe)
RATE_LIMIT_MINUTE = 65

def get_next_available_key():
    """Finds and returns an API key that is not currently rate-limited or has recovered."""
    
    # First, check if any previously rate-limited keys have recovered
    current_time = time.time()
    recovered_keys = []
    for key, timestamp in list(rate_limited_keys.items()):
        if current_time - timestamp > RATE_LIMIT_MINUTE:
            recovered_keys.append(key)
            
    # Remove recovered keys from the rate-limited dictionary
    for key in recovered_keys:
        del rate_limited_keys[key]
        print(f"   [API Key {key[:10]}... has recovered and is now available.]")

    # Now, find an available key from the full list
    available_keys = [key for key in API_KEYS if key not in rate_limited_keys]
...
def handle_rate_limit(api_key):
    """Adds a key to the rate-limited set with a timestamp."""
    print(f"   [API Key {api_key[:10]}... hit its limit. Switching keys.]")
    # Store the current time when the key was limited
    rate_limited_keys[api_key] = time.time()
...
def analyze_energy():
    ...
    while True:
        ...
        if not current_key:
            ...
            error_message = (
                "WattBot AI is currently experiencing high demand and has reached its temporary usage limit. "
                "Please wait a moment and try again."
            )
            return jsonify({
                "error": error_message, 
                "retry_after": RATE_LIMIT_MINUTE, # Suggest retry time in seconds
